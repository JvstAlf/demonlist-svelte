<script>
  ...
  let position = $state(match ? Number(match[1]) : null);
  ...
  async function loadDemon() {
    ...
    try {
      ...
      video = videoMatch ? `https://www.youtube.com/embed/${videoMatch[1]}` : null;

      const [fullResponse, levelResponse] = await Promise.all([
        fetch(`https://pointercrate.com/api/v2/demons/${demon.id}/`),
        fetch(`https://gdbrowser.com/api/level/${demon.level_id}/`)
      ]);

      const fullLevelInfo = await fullResponse.json();
      fullLevelData = fullLevelInfo.data;
      creators = fullLevelData.creators.map(c => c.name).join(', ');
...
{:else if ready && demon && fullLevelData && levelData}
  <main>
